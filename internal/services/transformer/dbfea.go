...
type dbfeaTransformer struct {
	baseWalletTransactionTransformer
}

func (t dbfeaTransformer) Transform(ctx context.Context, amount models.Amount, parentWalletTransaction models.WalletTransaction) (res []models.TransactionReq, err error) {
	status, err := transformWalletTransactionStatus(parentWalletTransaction.Status)
	if err != nil {
		return nil, err
	}

	lan := getLoanAccountNumberMetadata(parentWalletTransaction.Metadata)
	if lan == "" {
