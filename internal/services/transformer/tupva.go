...
// tupvaTransformer is a transformer to acuan transaction for TUPVA transaction type
type tupvaTransformer struct {
	baseWalletTransactionTransformer
}

func (t tupvaTransformer) Transform(_ context.Context, amount models.Amount, parentWalletTransaction models.WalletTransaction) (res []models.TransactionReq, err error) {
	...
	for _, detail := range parentWalletTransaction.Amounts {
		isAdminFee := slices.Contains([]string{"TUPVU", "ADMME"}, detail.Type)
		if isAdminFee {
