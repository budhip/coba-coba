...
// rpybvTransformer to handle mapping transaction CASH REPAYMENT BM validation (Group Loan)
type rpybvTransformer struct {
	baseWalletTransactionTransformer
}

func (t rpybvTransformer) Transform(_ context.Context, amount models.Amount, parentWalletTransaction models.WalletTransaction) (res []models.TransactionReq, err error) {
	...
	repaymentDate, ok := parentWalletTransaction.Metadata["repaymentDate"].(string)
	if !ok || repaymentDate == "" {
