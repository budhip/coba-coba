...
type dbfaaTransformer struct {
	baseWalletTransactionTransformer
}

func (t dbfaaTransformer) Transform(ctx context.Context, amount models.Amount, parentWalletTransaction models.WalletTransaction) (res []models.TransactionReq, err error) {
	...
	// TODO: product team still waiting from BU about this metadata
	// 		 so we can't use this metadata for now
	//
	//loanAccountNumber, ok := parentWalletTransaction.Metadata["loanAccountNumber"].(string)
	//if !ok || loanAccountNumber == "" {
	//	return nil, common.ErrMissingLoanAccountNumberFromMetadata
	//}
	//
	//agreementNumber, ok := parentWalletTransaction.Metadata["agreementNumber"].(string)
	//if !ok || agreementNumber == "" {
	//	return nil, common.ErrMissingAgreementNumberFromMetadata
	//}
