...
type rpyakTransformer struct {
	baseWalletTransactionTransformer
}

func (t rpyakTransformer) Transform(_ context.Context, amount models.Amount, parentWalletTransaction models.WalletTransaction) (res []models.TransactionReq, err error) {
	status, err := transformWalletTransactionStatus(parentWalletTransaction.Status)
	if err != nil {
		return nil, err
	}

	amarthaRevenue, err := getAccountNumberFromConfig(t.config.AccountConfig.AmarthaRevenueLoan, parentWalletTransaction.Description)
	if err != nil {
		return nil, err
	}
