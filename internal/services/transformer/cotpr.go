...
type cotprTransformer struct {
	baseWalletTransactionTransformer
}

func (t cotprTransformer) Transform(_ context.Context, amount models.Amount, parentWalletTransaction models.WalletTransaction) (res []models.TransactionReq, err error) {
	...
	for _, detail := range parentWalletTransaction.Amounts {
		isAdminFee := slices.Contains([]string{"ADMFE", "ADMCE"}, detail.Type)
		if isAdminFee {
