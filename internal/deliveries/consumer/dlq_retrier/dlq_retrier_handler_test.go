...
func newDlqRetrierHandlerHelper(t *testing.T) transactionHandlerHelper {
	t.Helper()
	t.Parallel()
...
func TestNewDLQRetrierHandler(t *testing.T) {
	th := newDlqRetrierHandlerHelper(t)
	...
	tests := []struct {
		name string
		args args
		want *DLQRetrierHandler
	}{
		{
			name: "success init DLQRetrierHandler",
			args: args{
				cfg: cfg,
				dp:  th.dp,
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			assert.Equalf(t, tt.want, NewRetrierHandler("", tt.args.dp, tt.args.cfg.MessageBroker.KafkaConsumer, nil), "NewTransactionHandler(%v)", tt.args.dp)
		})
	}
}
...
func TestTransactionHandler_processMessage(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			doMock: func(a args) {
				th.dp.EXPECT().RetryCreateOrderTransaction(gomock.Any(), gomock.Any()).Return(nil)
			},
			wantErr: false,
		},
		{
			...
			doMock: func(a args) {
				th.dp.EXPECT().RetryAccountMutation(gomock.Any(), gomock.Any()).Return(nil)
			},
			wantErr: false,
		},
		{
		...
		},
		{
			...
			doMock: func(a args) {
				th.dp.EXPECT().RetryCreateOrderTransaction(gomock.Any(), gomock.Any()).Return(assert.AnError)
			},
			wantErr: true,
		},
	}
