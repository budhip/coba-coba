...
type Consumer struct {
	...
	TransactionStreamHandler sarama.ConsumerGroupHandler
...
func (c *Consumer) preStart() error {
	...
	if c.consumerCfg.TopicTransactionStream == "" {
		return errors.New("no topics given to be consumed, please set the topic")
	}

	if c.consumerCfg.ConsumerGroupTransactionStream == "" {
		return errors.New("no kafka consumer group defined, please set the group")
	}

	// prometheus metrics
	if c.metrics != nil {
		c.consumerMetrics = metrics.NewConsumerMetrics(c.consumerCfg.ConsumerGroupTransactionStream, c.cfg.App.Name, 1*time.Second, c.metrics.PrometheusRegisterer())
		c.consumerMetrics.Run()
	}

	c.clientID = saramaCfg.ClientID
	c.TransactionStreamHandler = NewTransactionStreamHandler(c.clientID, c.mfs, c.dlq, c.cfg, c.consumerMetrics)

	client, err := sarama.NewConsumerGroup(c.consumerCfg.Brokers, c.consumerCfg.ConsumerGroupTransactionStream, saramaCfg)
	if err != nil {
		return err
	}
	c.cg = client
...
func (c *Consumer) Start() graceful.ProcessStarter {
	return func() error {
		...
		eg.Go(func() error {
			for {
				if err := c.cg.Consume(ctx, []string{c.consumerCfg.TopicTransactionStream}, c.TransactionStreamHandler); err != nil {
