...
func New(app fiber.Router, transactionSrv services.TransactionService, m middleware.AppMiddleware) {
	...
	finSnapshot := app.Group("/fin-snapshot/collect")
	finSnapshot.Get("/repayment-report", handler.finSnapshotCollectRepayment())
}
...
// @Router /v1/fin-snapshot/collect/repayment-report [get]
